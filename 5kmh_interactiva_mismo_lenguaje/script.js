...
const heroPixelWord = document.querySelector('#heroPixelWord');
...
function buildHeroWord() {
  if (!heroPixelWord) return;

  const heroColor = '#f5ead4';
  const shuffledIndexes = baseSlots
    .map((_, index) => index)
    .sort(() => Math.random() - 0.5);
  const delayByIndex = new Map();

  shuffledIndexes.forEach((slotIndex, order) => {
    delayByIndex.set(slotIndex, order * 8);
  });

  heroPixelWord.innerHTML = '';

  baseSlots.forEach((slot, index) => {
    const overlap = 1.8;
    const rect = makeSvgElement('rect', {
      class: 'hero-logo-tile',
      x: slot.x - overlap / 2,
      y: slot.y - overlap / 2,
      width: slot.w + overlap,
      height: slot.h + overlap,
      rx: 0,
      ry: 0,
      fill: heroColor
    });
    rect.style.setProperty('--build-delay', `${delayByIndex.get(index)}ms`);
    heroPixelWord.appendChild(rect);
  });

  const columns = 11;
  const rows = 4;
  const buckets = Array.from({ length: columns * rows }, () => []);
  const maxX = Math.max(...baseSlots.map(slot => slot.x + slot.w));
  const maxY = Math.max(...baseSlots.map(slot => slot.y + slot.h));
  const activeAliveIndexes = new Set();
  const recentAliveIndexes = [];
  const buildStartedAt = Date.now();

  baseSlots.forEach((slot, index) => {
    const column = Math.min(columns - 1, Math.floor(((slot.x + slot.w / 2) / maxX) * columns));
    const row = Math.min(rows - 1, Math.floor(((slot.y + slot.h / 2) / maxY) * rows));
    buckets[row * columns + column].push(index);
  });

  function pickAliveIndex() {
    const elapsed = Date.now() - buildStartedAt;
    const shuffledBuckets = buckets.filter(bucket => bucket.length).sort(() => Math.random() - 0.5);

    for (const bucket of shuffledBuckets) {
      const options = bucket.filter((index) => (
        !activeAliveIndexes.has(index)
        && !recentAliveIndexes.includes(index)
        && delayByIndex.get(index) < elapsed - 120
      ));
  ...
  function pulseNextTile() {
    const index = pickAliveIndex();
    if (index !== null) {
      const tile = heroPixelWord.children[index];
      activeAliveIndexes.add(index);
      recentAliveIndexes.push(index);
      if (recentAliveIndexes.length > 34) recentAliveIndexes.shift();
      tile.style.setProperty('--alive-scale', (0.32 + Math.random() * 0.18).toFixed(2));
      tile.classList.add('is-alive');
      window.setTimeout(() => {
        tile.classList.remove('is-alive');
        activeAliveIndexes.delete(index);
      }, 1650);
    }

    window.setTimeout(pulseNextTile, 95 + Math.random() * 165);
  }

  const fullBuildDuration = Math.max(...delayByIndex.values()) + 950;
  window.setTimeout(() => {
    window.setTimeout(() => {
      [...heroPixelWord.children].forEach(tile => tile.classList.add('is-built'));
    }, Math.max(0, fullBuildDuration - 900));
    pulseNextTile();
  }, 900);
}

function buildHeroModuleField() {
  const hero = document.querySelector('.hero');
  const field = document.querySelector('.hero-module-field');
  if (!hero || !field || !heroPixelWord || !baseSlots.length) return;

  const viewBoxWidth = 4054.03;
  const renderedWidth = heroPixelWord.getBoundingClientRect().width || 880;
  const scale = renderedWidth / viewBoxWidth;
  const sortedWidths = baseSlots.map(slot => slot.w).sort((a, b) => a - b);
  const medianWidth = sortedWidths[Math.floor(sortedWidths.length / 2)] || 42;
  const moduleSize = Math.max(7, Math.min(18, medianWidth * scale));
  const totalModules = window.matchMedia('(max-width: 640px)').matches ? 34 : 58;

  field.innerHTML = '';

  for (let index = 0; index < totalModules; index += 1) {
    const module = document.createElement('span');
    module.className = 'hero-bg-module';
    module.style.setProperty('--module-size', `${moduleSize.toFixed(2)}px`);
    module.style.setProperty('--module-x', `${(Math.random() * 100).toFixed(2)}%`);
    module.style.setProperty('--module-y', `${(Math.random() * 100).toFixed(2)}%`);
    module.style.setProperty('--module-delay', `${(-Math.random() * 7).toFixed(2)}s`);
    module.style.setProperty('--module-duration', `${(4.2 + Math.random() * 4.8).toFixed(2)}s`);
    module.style.setProperty('--module-opacity', `${(0.07 + Math.random() * 0.13).toFixed(2)}`);
